...
# 2. Cleaning up the Mycocosm 2024 database
## Problematic
...
To perform a Diamond blastX alignment of the mycocosm genomes against the mycocosm database, the CDS (coding sequence) and AA (amino acid) files of all the genomes in the MycoCosm 2024 database must be available. The clean-up of the MycoCosm 2024 database consisted in recovering a CDS file and a AA file stored for each MycoCosm genome in a directory.

## Diagnostic

The number of genome directories obtained after the downloading is obtained by running the bash command: 'ls | wc -l' after moving to the folder where they are stored. The command displays 2526 directories.
The CDS and AA files for each genome have a '.fasta.gz' extension. The number of files with a '.fasta.gz' extension for the 2526 genome directories can be obtained using the command : ls (path) | grep -E '.fasta.gz' | wc -l.
...
find (path) -mindepth 1 -type d -exec sh -c 'test $(find “{}” -type f -name “”*.fasta.gz” | wc -l) -ge x ‘ ; -print | wc -l, specifying the minimum number of ’.fasta.gz‘ files that the directories must contain in place of ’x‘ in the ’-ge x' argument,
...
This can be obtained directly with the bash command: find (path) -mindepth 1 -type d -exec sh -c 'test -z “$(find ‘{}’ -type f -name ‘*.fasta.gz’ -print -quit)”' -print | wc -l
...
We copy all the files in the sub-folders of the directory where genomes were dowloaded to another folder, by running the copy.py program.
You can check that the number of files copied is 5045 for example by means of a variable n in the script, which is incremented by 1 each time a file is copied and displayed at the end of program execution. You can also run the command 'ls | wc -l' in the destination folder.
...
This information can be obtained by executing the command: ls (path)/**/*.fasta.gz | grep -vE 'aa.fasta.gz' | wc -l to obtain the number of files not ending in 'aa.fasta.gz'. 
Simply remove the 'v' from the 'grep' argument to obtain the number of files ending in 'aa.fasta.gz'.
By executing the command: ls (path)/**/*.fasta.gz | grep -E '*CDS.*\.fasta.gz' | wc -l, we can see that 2498 files that didn't end in 'aa.fasta.gz' are CDS files with a variable name nomenclature, but always containing 'CDS'.
Finally, using the command: ls (path)/2_files_fasta.gz/**/*.fasta.gz | grep -vE 'aa.fasta.gz|.*CDS.*\.fasta.gz' and without adding the 'wc -l' option, we obtain the number of files with names not ending in 'fasta.gz' and not containing 'CDS'.
...
We apply the copy_create.py program, which copies the relevant sub-directories with their files from (path)/all_fasta.gz_files/ to (path)/2_files_fasta.gz/.
...
and use the command: find (path) -mindepth 1 -type d -exec sh -c 'test $(find “{}” -type f -name “*.fasta.gz” | wc -l) -ge 2 ' ; -print | wc -l to ensure, by varying the argument of the 'ge' option from 2 to 6, that all directories contain only 
2 fasta.gz files, after checking that none contain 1 or 0.

## For directories containing more than 2 fasta.gz files

There are 16 directories containing more than 2 fasta.gz files. We can display their paths using the command used previously: find (path) -mindepth 1 -type d -exec sh -c 'test $(find “{}” -type f -name “*.fasta.gz” | wc -l) -ge 2 ' \; -print,
...
It will copy from the folder all_files_fasta.gz to (path)/2_more_files.fasta.gz, the files in the directories concerned and create a directory in their name in which to store their files.
...
# 3. Order of use for the scripts
...
a table showing the correspondance between transcriptID and protID of all genes for all genomes
...
This script will order the KOG annotation files of all genomes by their protID, which is very important before
...
The script will use the "protID dictionnaries" generated in the previous step to creates 2 files for each genomes that associate to each hits of their genes
their KOG annotations and their SigP annotations.

### KOG_SiGP_queries.py

This one will then from the files generated add the KOG and SigP annotations of the queries. It use "transcriptID dictionnaries".
...
Theses scripts will use the transformed fungaltrait database. They will calculate for each diamonds outputs with queries and hits linked to their KOG and SignalP annotations the percentages of correct taxonomic, trophic and
